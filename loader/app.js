...
// ── 0. API config ─────────────────────────────────────────────────────────────
function apiBase() {
  return window.IMMORTAL_API || '';
}
const IMMORTAL_PORTAL = window.IMMORTAL_PORTAL || '#';
const IMMORTAL_STATIC = () => window.IMMORTAL_STATIC === true || !apiBase();
document.addEventListener('DOMContentLoaded', () => {
  ...
  paintApiBadge('ok', 'STATIC SITE');
});

async function apiFetch(path, opts = {}, timeoutMs = 12000) {
  if (IMMORTAL_STATIC()) throw new Error('static');
...
function paintApiBadge(state, detail) {
  ...
  el.textContent = detail || (state === 'ok' ? 'ONLINE' : state === 'down' ? 'OFFLINE' : '…');
}
...
async function probeApiHealth() {
  ...
  paintApiBadge('pending', '…');
  try {
    const res = await apiFetch('/health', {}, 4000);
    const data = await res.json().catch(() => ({}));
    if (data.status === 'degraded' || data.db === 'down') {
      paintApiBadge('pending', data.version ? `${data.version} · DB?` : 'DEGRADED');
      return false;
    }
    if (!res.ok) throw new Error('bad');
    paintApiBadge('ok', data.version ? String(data.version) : 'ONLINE');
    return true;
  } catch (_) {
    paintApiBadge('down', 'OFFLINE');
    return false;
  }
}
...
async function doLogin() {
  ...
  try {
    // Pure static GitHub Pages — no backend
    if (IMMORTAL_STATIC()) {
      await new Promise((r) => setTimeout(r, 500));
      const token = 'static-' + key;
      Tokens.save(token, token);
      try { localStorage.setItem('immortal_last_key', key); } catch (_) {}
      paintApiBadge('ok', 'STATIC SITE');
      nativeSend({ action: 'authOk', key, username: 'Guest', expiry: 'Lifetime', token });
      onAuthOk({
        username: 'Guest',
        expiry: 'Lifetime',
        products: [{ id: 'private', name: 'Immortal Private' }, { id: 'emu', name: 'Emulator' }],
      });
      setAuthBtnLoading(false);
      return;
    }
    ...
    const msg =
      ...
      : err.message === 'Failed to fetch' ? 'Cannot reach server'
